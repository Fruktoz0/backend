...
router.post('/login', loginLimiter, async (req, res) => {
    ...
    //Fiók státuszának ellenőrzése        
        ...
        if (user.isActive === "archived") {
            return res.status(403).json({ message: "A felhasználó archiválva" }).end()
        }
        if (user.isActive === "inactive") {
            return res.status(403).json({ message: 'A fiók inaktív, kérlek erősítsd meg az emailed.' }).end()
        }
...
router.delete('/delete/:email', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(402).json({ message: 'Felhasználó nem található.' }).end()
        }
