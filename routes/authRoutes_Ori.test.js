...
describe('Login of "auth" route:', () => {
    ...
    test('Login Admin Hiányzó adat s01 - 400 [400]', async () => {
        const response = await (server).post('/api/auth/login').send({ password: 'admin' })
        expect(response.statusCode).toBe(400)
    })
    test('Login Admin Hiányzó adat s10 - 400 [400]', async () => {
        const response = await (server).post('/api/auth/login').send({ email: 'admin@admin.hu' })
        expect(response.statusCode).toBe(400)
    })
})
...
describe('Login of "auth" route:', () => {
    test('Login Admin Wrong Password - 401 [401]', async () => {
        const response = await (server).post('/api/auth/login').send({ email: 'admin@admin.hu', password: 'adminG' })
...
describe('Login of "auth" route:', () => {
    test('Login Admin Wrong Email - 404 [404]', async () => {
        const response = await (server).post('/api/auth/login').send({ email: 'admin@admin.huG', password: 'admin' })
...
describe('Login of "auth" route:', () => {
    test('Login as Admin     s11 [200]', async () => {
        const response = await (server).post('/api/auth/login').send({ email: 'admin@admin.hu', password: 'admin' })
...
describe('Login of "auth" route:', () => {
    test('Login as User     s11 [200]', async () => {
        const response = await (server).post('/api/auth/login').send({ email: 'zoardakarki@gmail.com', password: '123456789' })
        expect(response.statusCode).toBe(200)
...
describe('User Data of "auth" route:', () => {
    test('GetUser Data with token_admin [200]', async () => {
        const response = await (server).get('/api/auth/user')
        .set('Authorization', 'bearer ' + token_admin)
        expect(response.statusCode).toBe(200)
...
describe('Register of "auth" route All data:', () => {
    ...
    test('Register User Hiányzó adat s0001 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0010 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ password: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0011 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ password: 'Meki#012345', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0100 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ email: 'mindent_is_tudok@gmail.com' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0101 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ email: 'mindent_is_tudok@gmail.com', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0110 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ email: 'mindent_is_tudok@gmail.com', password: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s0111 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ email: 'mindent_is_tudok@gmail.com', password: 'Meki#012345', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1000 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1001 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1010 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', password: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1011 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', password: 'Meki#012345', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1100 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', email: 'mindent_is_tudok@gmail.com' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1101 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', email: 'mindent_is_tudok@gmail.com', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
    test('Register User Hiányzó adat s1110 Mekk Elek [400]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', email: 'mindent_is_tudok@gmail.com', password: 'Meki#012345' })
        expect(response.statusCode).toBe(400)
    })
})
...
describe('Register of "auth" route All data:', () => {
    test('Register User adat s111 Mekk Elek [201]', async () => {
        const response = await (server).post('/api/auth/register').send({ username: 'Mekk Elek', email: 'mindent_is_tudok@gmail.com', password: 'Meki#012345', confirmPassword: 'Meki#012345' })
        expect(response.statusCode).toBe(201)
    })
})
